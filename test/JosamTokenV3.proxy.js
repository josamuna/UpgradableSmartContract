...
describe("ERC20 Josam Token proxy version 3.", function () {
  before(async function () {
    JosamTokenV2 = await ethers.getContractFactory("JosamTokenV2");
    JosamTokenV3 = await ethers.getContractFactory("JosamTokenV3");

    josamtokenV2 = await upgrades.deployProxy(
      JosamTokenV2,
      ["Josam Token", "JTK"],
      { initializer: "initialize" }
    ); // Upgrade contract version 2.

    josamtokenV3 = await upgrades.upgradeProxy(
      josamtokenV2.address,
      JosamTokenV3
    ); // Upgrade contract V3 using V2 Contract address.
  ...
  // THE REST OF THE TEST WILL NOT BE WORK BECAUSE IN THE PROXY THE OWNER IS ADDRESS (0) => CONTRACT CALL ANOTHER ONE
  // THE CONTRACT V2 INITIALIZE THE CONTRACT V3 AND WE WERE NOT ABLE TO CHANGE THE OWNER (ADDRESSS(0))
